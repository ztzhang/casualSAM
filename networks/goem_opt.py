...
def reproject_depth(src_R, src_t, src_disp, tgt_R, tgt_t, tgt_disp, K, K_inv, coord, eps=1e-6):
    """
    ...
    src_xyz = src_depth_flat * src_R.matmul(K_inv.matmul(coord)) + src_t
    src_xyz_at_tgt_cam = K.matmul(
    ...
    tgt_xyz = tgt_depth_flat * tgt_R.matmul(K_inv.matmul(coord)) + tgt_t
    tgt_xyz_at_src_cam = K.matmul(
...
# @torch.jit.script
def warp_by_disp(src_R, src_t, tgt_R, tgt_t, K, src_disp, coord, inv_K, debug_mode=False):
...
class DepthBasedWarping(nn.Module):
    ...
    def reproject_depth(self, src_R, src_t, src_disp, tgt_R, tgt_t, tgt_disp, K, K_inv, eps=1e-6, check_shape=False):
        if check_shape:
            check_R_shape(src_R)
            check_R_shape(tgt_R)
            check_t_shape(src_t)
        ...
        return reproject_depth(src_R, src_t, src_disp, tgt_R, tgt_t, tgt_disp, K, K_inv, self.coord, eps=eps)
    ...
    def forward(
        ...
        return warp_by_disp(src_R, src_t, tgt_R, tgt_t, K, src_disp, self.coord, inv_K, debug_mode)
